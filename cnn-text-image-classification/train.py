...
import data_helpers
from text_image_cnn import TextImageCNN
...
# 数据参数
flags.DEFINE_string('image_directory', './data/image/',
                    'Directory path of the image data')
flags.DEFINE_string('text_directory', './data/text/',
                    'Directory path of the text data')
flags.DEFINE_float('validation_percentage', 0.1,
                   'Percentage of the training data to use for validation')
flags.DEFINE_integer('image_size', 128, 'Image size for resizing')
...
flags.DEFINE_float('l2_reg_lambda_im', 0.0001, 'Lambda for L2 regularizer')
...
flags.DEFINE_float('l2_reg_lambda_end', 0.0,
...
# 加载数据
text_train, text_valid, image_train, image_valid, y_train, y_valid, vocab_processor = data_helpers.load_train_valid_data(
    FLAGS.text_directory, FLAGS.image_directory, FLAGS.validation_percentage,
    FLAGS.image_size)
...
cnn = TextImageCNN(
    ...
    l2_reg_lambda_end=FLAGS.l2_reg_lambda_end,
    ...
    l2_reg_lambda_im=FLAGS.l2_reg_lambda_im)
...
with tf.Session() as sess:
    ...
    def train_step(im_batch, txt_batch, y_batch):
        feed_dict = {
            cnn.input_x_txt: im_batch,
            cnn.input_x_im: txt_batch,
            cnn.input_y: y_batch,
    ...
    def valid_step(im_batch, txt_batch, y_batch, writer=None):
        feed_dict = {
            cnn.input_x_txt: im_batch,
            cnn.input_x_im: txt_batch,
            cnn.input_y: y_batch,
    ...
    batches = data_helpers.batch_iter(
        list(zip(text_train, image_train, y_train)), FLAGS.batch_size,
        FLAGS.num_epochs)

    # 迭代训练每个batch
    for batch in batches:
        txt_batch, im_batch, y_batch = zip(*batch)
        train_step(txt_batch, im_batch, y_batch)
        current_step = tf.train.global_step(sess, global_step)
        if current_step % FLAGS.evaluate_every == 0:
            print('\nEvaluation:')
            valid_step(
                text_valid, image_valid, y_valid, writer=valid_summary_writer)
            print('')
